...
추론은 학습된 모델을 사용하여 새로운 데이터를 예측하는 과정입니다. 이 과정은 계산량이 많을 수 있기 때문에, 전용 서버에서 실행하는 것이 흥미로운 옵션이 될 수 있습니다. `old_huggingface_hub` 라이브러리는 호스팅된 모델에 대한 추론을 실행하는 간단한 방법을 제공합니다. 연결할 수 있는 서비스는 여러가지가 있습니다:
...
## 추론 클라이언트[[old_huggingface_hub.InferenceClient]]

[[autodoc]] InferenceClient

## 비동기 추론 클라이언트[[old_huggingface_hub.AsyncInferenceClient]]
...
## 추론 시간 초과 오류[[old_huggingface_hub.InferenceTimeoutError]]
...
### 대화형 출력 대화[[old_huggingface_hub.inference._types.ConversationalOutputConversation]]

[[autodoc]] old_huggingface_hub.inference._types.ConversationalOutputConversation

### 대화형 출력[[old_huggingface_hub.inference._types.ConversationalOutput]]

[[autodoc]] old_huggingface_hub.inference._types.ConversationalOutput

### 모델 상태[[old_huggingface_hub.inference._common.ModelStatus]]

[[autodoc]] old_huggingface_hub.inference._common.ModelStatus

## 추론 API[[old_huggingface_hub.InferenceApi]]
