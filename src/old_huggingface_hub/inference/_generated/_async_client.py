...
#
# WARNING
# This entire file has been adapted from the sync-client code in `src/old_huggingface_hub/inference/_client.py`.
...
from old_huggingface_hub.constants import ALL_INFERENCE_API_FRAMEWORKS, INFERENCE_ENDPOINT, MAIN_INFERENCE_API_FRAMEWORKS
from old_huggingface_hub.errors import InferenceTimeoutError
...
from old_huggingface_hub.inference._generated.types import (
...
from old_huggingface_hub.inference._generated.types.chat_completion import ChatCompletionInputToolTypeEnum
from old_huggingface_hub.inference._types import (
...
class AsyncInferenceClient:
    ...
    async def chat_completion(
        ...
        *,
        ...
        have `minijinja` template engine installed. Run `pip install "old_huggingface_hub[inference]"` or `pip install minijinja`
        to install it.
    ...
    async def conversational(
        ...
        *,
        ...
        """
        warnings.warn(
            "'InferenceClient.conversational' is deprecated and will be removed starting from old_huggingface_hub>=0.25. "
    ...
    async def text_generation(
        ...
        *,
        ...
        Returns:
            ...
            - if `stream=False` and `details=True`, the generated text is returned with more details as a [`~old_huggingface_hub.TextGenerationOutput`]
            - if `details=True` and `stream=True`, the generated text is returned token by token as a iterable of [`~old_huggingface_hub.TextGenerationStreamOutput`]
        ...
        >>> await client.text_generation("The old_huggingface_hub library is ", max_new_tokens=12)
        '100% open source and built to be easy to use.'

        # Case 2: iterate over the generated tokens. Useful async for large generation.
        >>> async for token in await client.text_generation("The old_huggingface_hub library is ", max_new_tokens=12, stream=True):
        ...     print(token)
        100
        %
        open
        ...
        >>> await client.text_generation("The old_huggingface_hub library is ", max_new_tokens=12, details=True)
        TextGenerationOutput(
        ...
        >>> async for details in await client.text_generation("The old_huggingface_hub library is ", max_new_tokens=12, details=True, stream=True):
        ...     print(details)
        ...
