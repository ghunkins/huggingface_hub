...
def is_pydantic_available() -> bool:
    ...
    # it is installed with `pip install old_huggingface_hub[inference]` but it can happen when it is installed manually
    ...
    # as it is automatically imported when doing `from old_huggingface_hub import ...` even if the user doesn't use it.
    #
    # See https://github.com/huggingface/old_huggingface_hub/pull/1829 for more details.
    try:
        from pydantic import validator  # noqa: F401
    except ImportError:
        # Example: "ImportError: cannot import name 'TypeAliasType' from 'typing_extensions'"
        warnings.warn(
            "Pydantic is installed but cannot be imported. Please check your installation. `old_huggingface_hub` will "
...
try:
    ...
    # warning disappear. See https://github.com/huggingface/old_huggingface_hub/issues/1043
    #
...
def dump_environment_info() -> Dict[str, Any]:
    ...
    """
    from old_huggingface_hub import get_token, whoami
    from old_huggingface_hub.utils import list_credential_helpers

    token = get_token()

    # Generic machine info
    info: Dict[str, Any] = {
        "old_huggingface_hub version": get_hf_hub_version(),
