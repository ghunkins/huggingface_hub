...
"""Utility helpers to handle progress bars in `old_huggingface_hub`.

Example:
    1. Use `old_huggingface_hub.utils.tqdm` as you would use `tqdm.tqdm` or `tqdm.auto.tqdm`.
...
Example:
    ```py
    >>> from old_huggingface_hub.utils import are_progress_bars_disabled, disable_progress_bars, enable_progress_bars, tqdm
...
class tqdm(old_tqdm):
    ...
    def __delattr__(self, attr: str) -> None:
        """Fix for https://github.com/huggingface/old_huggingface_hub/issues/1603"""
        try:
...
def tqdm_stream_file(path: Union[Path, str]) -> Iterator[io.BufferedReader]:
    ...
    First implemented in `transformers` in 2019 but removed when switched to git-lfs. Used in `old_huggingface_hub` to show
